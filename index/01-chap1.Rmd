...
As an exploration of the use of the DADA2 pipeline for the processing of microbiome sequencing data, we used the European Nucleotide Archive [@Harrison2018] to download raw read data from Catherine Lozupone's 2013 study on gut-linked diseases prevalent in HIV [@Lozupone2013]. With these data, we applied Benjamin Callahan's 2016 paper, "Bioconductor Workflow for Microbiome Analysis: from raw reads to community analyses," to manipulate the Lozupone sequencing data [@Callahan2016a]. With this pipeline, we visualized the fastq quality scores of our read files (Figure \@ref(fig:figure1)) to trim our input reads at ideal positions. We also filtered reads through the enforcement of a maximum of 2 expected errors per read. 
